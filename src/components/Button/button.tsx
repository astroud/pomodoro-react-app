...
const Button: React.FC<ButtonProps> = (props: ButtonProps) => {
  if(props.buttonType === 'settings') {
    return(
      <button className="pomodoro-app__preferences" name="settings" title="show preferences pane" onClick={props.toggleVisibility}>
  ...
  if(props.buttonType === 'close') {
    return(<button className="pane__close-preferences" onClick={props.toggleVisibility}>{props.buttonText}</button>)
  }

  if(props.buttonType === 'apply') {
    return(
      <div className="pane__apply-row">
        <input type="submit" value={props.buttonText} className="pane__apply-preferences" />
      </div>
    )
  }
